...
class PaperDatabase:
    def __init__(self, output_dir: str = "dataset"):
        ...
        self.filename = os.path.join(output_dir, "rss21.json")
...
def scrape_individual_paper(session, paper_url):
    ...
    """
    paper_url = paper_url.replace("org", "org/2021/program/papers")
    ...
    # Find the PDF link
    pdf_link = None
    img_tag = soup.find('img', alt='Paper Website')
    if img_tag:
        pdf_a_tag = img_tag.find_parent('a')
        if pdf_a_tag and pdf_a_tag.get('href'):
            pdf_link = urllib.parse.urljoin(BASE_URL, pdf_a_tag.get('href'))

    # Extract Abstract
    abstract = None
    abstract_heading = soup.find(id='abstract')
    if abstract_heading:
...
def scrape_rss_papers():
    """
    Scrapes the RSS 2021 papers and saves them to a JSON database.
    """    
    db = PaperDatabase()
...
if __name__ == "__main__":
    ...
    PAPERS_URL = "https://roboticsconference.org/2021/program/papers/"
    scrape_rss_papers()