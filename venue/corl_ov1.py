...
class PaperDatabase:
    def __init__(self, output_dir: str = "dataset", Year: int = 2023):
        self.output_dir = output_dir
        self.filename = os.path.join(output_dir, f"corl{str(Year)[2:]}.json")
...
def scrape_corl_papers(username: str, Year: int):
    """
    Scrapes the CoRL papers using OpenReview API and saves them to a JSON database.
    ...
    try:
        # --- API v1 client for CoRL 2023 ---
        client = openreview.Client(
            baseurl='https://api.openreview.net',
            username=username,
            password=input('OpenReview password: ')
        )

        venue_id = f'robot-learning.org/CoRL/{Year}/Conference'

        # 1) All submissions (v1 uses Blind_Submission)
        submissions = list(openreview.tools.iterget_notes(
            client, invitation=f'{venue_id}/-/Blind_Submission'
        ))
...
if __name__ == "__main__":
    ...
    scrape_corl_papers(USERNAME, Year)